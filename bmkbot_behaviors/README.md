# FlexBE States and Behaviors for bmkbot
...
This raw repository has several folders and files with the generic name `bmkbot`.
...
and change the relevant `bmkbot` text to `my_new_project_name` as needed.
...
## Example States in `bmkbot_flexbe_states`
...
## Example Behaviors in `bmkbot_flexbe_behaviors`
...
[`copy_behavior`](bmkbot_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run bmkbot_flexbe_behavior copy_behavior` to see the usage guide. 
